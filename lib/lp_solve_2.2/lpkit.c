  ...
  for(i = 1; i <= lp->rows; i++)
	...
	printf("% 8.2f", lp->orig_rh[i]);
      if(lp->orig_lowbo[i] != 0) {
        printf("  %s=%8.2f", (lp->ch_sign[i]) ? "lowbo" : "upbo",
	       lp->orig_lowbo[i]);
      }
      if((lp->orig_upbo[i]!=lp->infinite) && (lp->orig_upbo[i]!=0.0)) {
        printf("  %s=%8.2f", (lp->ch_sign[i]) ? "upbo" : "lowbo",
	       lp->orig_upbo[i]);
      }
      printf("\n");
    }
...
void print_solution(lprec *lp)
{
  ...
  for(i = 1; i <= lp->columns; i++)
    if(lp->names_used)
      fprintf(stream, "%-10s%16.10g\n", lp->col_name[i],
	      (double)lp->best_solution[lp->rows+i]);
    else
      fprintf(stream, "Var [%4d]  %16.10g\n", i,
	      (double)lp->best_solution[lp->rows+i]);
...
void write_MPS(lprec *lp, FILE *output)
{
  ...
  for(i = 1; i <= lp->columns; i++)
      ...
      if(lp->maximise)
	{
	  if(column[j] != 0)
	    { 
	      if(lp->names_used)
		fprintf(output, "    %-8s  %-8s  %1.10g\n", lp->col_name[i],
			lp->row_name[j], -column[j]);
	      else
		fprintf(output, "    var_%-4d  r_%-6d  %1.10g\n", i, j,
			-column[j]);
	    }
	} 
      else
  ...
  for(i = 1; i <= lp->rows; i++)
	...
	{
	  if(lp->names_used)
	    fprintf(output, "    RHS       %-8s  %1.10g\n", lp->row_name[i],
		    (double)-a);
	  else
	    fprintf(output, "    RHS       r_%-6d  %1.10g\n", i,
		    (double)-a);
	}
      else
	{
  ...
  for(i = 1; i <= lp->rows; i++)
	...
	a /= lp->scale[i];
      if(lp->names_used)
	fprintf(output, "    RGS       %-8s  %1.10g\n", lp->row_name[i],
		(double)a);
      else
	fprintf(output, "    RGS       r_%-6d  %1.10g\n", i,
		(double)a);
    }
    ...
    for(i = lp->rows + 1; i <= lp->sum; i++)
      {
        if((lp->orig_lowbo[i] != 0) && (lp->orig_upbo[i] < lp->infinite) &&
           (lp->orig_lowbo[i] == lp->orig_upbo[i]))
          {
            ...
            if(lp->scaling_used)
              a *= lp->scale[i];
            fprintf(output, " FX BND       %-8s  %1.10g\n",
        ...
        else
          {
            if(lp->orig_upbo[i] < lp->infinite) {
              ...
              if(lp->scaling_used)
                a *= lp->scale[i];
              fprintf(output, " UP BND       %-8s  %1.10g\n",
    		      lp->col_name[i- lp->rows], (double)a);
            }
    	    if(lp->orig_lowbo[i] != 0) {
              a = lp->orig_lowbo[i];
              if(lp->scaling_used)
                a *= lp->scale[i];
    	      fprintf(output, " LO BND       %-8s  %1.10g\n",
        ...
        if((lp->orig_lowbo[i] != 0) && (lp->orig_upbo[i] < lp->infinite) &&
        ...
        else
          {
            if(lp->orig_upbo[i] < lp->infinite) {
              ...
              if(lp->scaling_used)
                a *= lp->scale[i];
              fprintf(output, " UP BND       var_%-4d  %1.10g\n",
    		      i - lp->rows, (double)a);
            }
            if(lp->orig_lowbo[i] != 0) {
              a = lp->orig_lowbo[i];
              if(lp->scaling_used)
                a *= lp->scale[i];
              fprintf(output, " LO BND       var_%-4d  %1.10g\n", i - lp->rows,
    		      (double)a);
            }
          }
      }
...
void write_MPS_by_queue(lprec *lp, FILE *output)
{
  int i, j, marker, putheader;
  REAL a;
  matrec *cur_row;
  extern matrec* lp_dequeue();
  ...
  for(i = 1; i <= lp->columns; i++)
      ...
      if((!lp->must_be_int[i+lp->rows]) && (marker % 2)==1)
	{
      ...
      get_column_in_queueu(lp, i);
      while(cur_row = lp_dequeue()){
        if(cur_row->row_nr == 0){ /* objective function */
	  j = 0;
          if(lp->maximise){
	      if(lp->names_used)
		fprintf(output, "    %-8s  %-8s  %1.10g\n", lp->col_name[i],
			lp->row_name[j], -cur_row->scaled_value);
	      else
		fprintf(output, "    var_%-4d  r_%-6d  %1.10g\n", i, j,
			-cur_row->scaled_value);
        }
        else
	{
	      if(lp->names_used)
